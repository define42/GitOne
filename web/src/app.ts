...
type RepositoryIssueState = "open" | "closed" | "all";

interface RepositoryIssueComment {
  id: number;
  author: string;
  body: string;
  createdAt: string;
...
interface RepositoryBrowserRoute {
  ...
  path: string;
  file: string | null;
  view: "files" | "history" | "builds" | "blame" | "merge-requests" | "issues";
  page: number;
  mergeRequest?: number;
  issue?: number;
  reviewTab: RepositoryMergeRequestTab;
  mergeRequestState: RepositoryMergeRequestState;
  issueState: RepositoryIssueState;
}
...
const logoutRoot = document.querySelector<HTMLButtonElement>("#logout");
...
const iconPaths: Record<IconName, string[]> = {
  ...
  repository: [
    // Bootstrap Icons "git", licensed under the MIT License.
...
function icon(name: IconName): SVGSVGElement {
  ...
  svg.setAttribute("viewBox", iconViewBoxes[name] ?? "0 0 24 24");
...
function repositoryDefaultBranchAPIURL(repository: string): string {
...
function repositoryIssuesAPIURL(
  repository: string,
  issue?: number,
): string {
...
function repositoryIssueCommentsAPIURL(
  repository: string,
  issue: number,
): string {
...
type GroupTreeResource =
  | {kind: "subgroup"; name: string; subgroup: GroupSummary}
  | {kind: "repository"; name: string; repository: RepositorySummary};

let groupTreeChildrenSequence = 0;

function groupTreeResources(data: GroupDetail): GroupTreeResource[] {
  const resources: GroupTreeResource[] = [
...
function groupTreeSpacer(): HTMLSpanElement {
...
function groupTreeItems(data: GroupDetail): HTMLLIElement[] {
  return groupTreeResources(data).map((resource) => {
    ...
    toggle.addEventListener("click", async () => {
      ...
      try {
        const nested = await request<GroupDetail>(apiGroupURL(subgroup.path));
        if (!item.isConnected) {
          return;
        }
...
function groupTree(data: GroupDetail): HTMLElement {
...
function repositoryRenameControl(
  ...
  const description = element(
    "p",
    "The repository URL will change. Git data, LFS objects, builds, and merge requests move with it.",
  );
  ...
  form.append(
    header,
    description,
    fieldLabel("New repository name", name),
    actions,
  );
  dialog.append(form);
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      window.location.assign(repositoryBrowserURL(
        `${groupPath}/${renamedName}`,
        {
          ref: route.ref,
          path: route.path,
      ...
      showStatus(
        reason instanceof Error ? reason.message : "Could not rename the repository.",
        true,
      );
      submit.disabled = false;
      cancel.disabled = false;
...
function repositoryDefaultBranchControl(
  route: RepositoryBrowserRoute,
  ...
  trigger.addEventListener("click", async () => {
    ...
    try {
      ...
      showStatus(
        reason instanceof Error ? reason.message : "Could not change the default branch.",
        true,
      );
...
function repositoryImportControl(
  ...
  type ImportMethod = "remote" | "archive";
  ...
  let suggestedName = "";
  const suggestName = (value: string): void => {
  ...
  const archiveHint = element(
    "p",
    "Upload up to 1 GiB. The bare repository can be at the archive root or inside one enclosing folder. Links and special files are rejected.",
  );
  ...
  archivePanel.append(
    fieldLabel("Bare repository archive", archiveFile),
    archiveHint,
  );
  ...
  let method: ImportMethod = "remote";
  const selectMethod = (next: ImportMethod, focus = true): void => {
    method = next;
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (method === "remote") {
        ...
        await request(
          `/api/repositories/${repositoryPath}/import-archive?filename=${encodeURIComponent(archive.name)}`,
          {
            method: "POST",
            headers: {
...
function fieldLabel<T extends HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | HTMLDivElement>(
  text: string,
  field: T,
): HTMLLabelElement {
...
const syntaxLanguagesByExtension: Record<string, string> = {
  bash: "bash",
  c: "c",
  cc: "cpp",
  conf: "ini",
...
const syntaxLanguagesByFilename: Record<string, string> = {
  ".bashrc": "bash",
  ".dockerignore": "git",
  ".editorconfig": "ini",
  ".gitattributes": "git",
...
function syntaxLanguageForPath(path: string): string | null {
...
function syntaxHighlightedHTML(content: string, path: string): string | null {
...
interface RepositoryPipeline {
  ...
  commit: string;
  createdAt: string;
  jobs: RepositoryBuild[];
}

function repositoryPipelines(builds: RepositoryBuild[]): RepositoryPipeline[] {
  const pipelines = new Map<string, RepositoryPipeline>();
...
function aggregateBuildStatus(jobs: RepositoryBuild[]): RepositoryBuildStatus {
  for (const status of [
    "failed",
    "running",
    "queued",
...
function pipelineStages(jobs: RepositoryBuild[]): RepositoryBuild[][] {
  const byID = new Map(jobs.map((job) => [job.id, job]));
  const depths = new Map<string, number>();
  const visiting = new Set<string>();
  const depth = (job: RepositoryBuild): number => {
    ...
    const dependencies = (job.needs ?? [])
      ...
      .filter((dependency): dependency is RepositoryBuild => dependency !== undefined);
...
function latestBranchBuildIndicator(
  ...
  const render = (): void => {
    ...
    link.setAttribute(
      "aria-label",
      `Latest pipeline on ${route.ref}: ${status} at ${shortCommitHash(build.commit)}`,
    );
  };
...
function repositoryBuildsView(
  ...
  let refreshing = false;
  let timer: number | undefined;
  let activeLogBuildID: string | null = null;
  let logTrigger: HTMLButtonElement | null = null;
  const logs = new Map<string, string>();
  const loadingLogs = new Set<string>();
  const mutatingBuilds = new Set<string>();
  let dependencyFrame: number | undefined;

  const drawDependencyLines = (): void => {
    dependencyFrame = undefined;
    for (const canvas of listRoot.querySelectorAll<HTMLElement>(".pipeline-graph-canvas")) {
      const svg = canvas.querySelector<SVGSVGElement>(".pipeline-dependencies");
      if (!svg) {
        continue;
      }
      ...
      for (const card of canvas.querySelectorAll<HTMLElement>(".build-item")) {
  ...
  const scheduleDependencyLines = (): void => {
  ...
  const openBuildLog = (build: RepositoryBuild, trigger: HTMLButtonElement): void => {
  ...
  logDialog.addEventListener("close", () => {
    ...
    const currentTrigger = closedBuildID === null
      ? null
      : listRoot.querySelector<HTMLButtonElement>(
        `.build-log-toggle[data-build-id="${CSS.escape(closedBuildID)}"]`,
      );
  ...
  const renderBuilds = (): void => {
    const graphScrollPositions = new Map<string, number>();
    for (const graph of listRoot.querySelectorAll<HTMLElement>(".pipeline-graph-scroll")) {
    ...
    if (data.builds.length === 0) {
      listRoot.replaceChildren(
        emptyState("No pipelines yet. Push a branch containing a .gitone.yaml build definition."),
      );
      return;
    }
    ...
    for (const pipeline of pipelines) {
      ...
      pipelineTitle.append(
        element("strong", `Pipeline · ${pipeline.branch}`),
        element("code", shortCommitHash(pipeline.commit)),
      );
      ...
      pipelineMetadata.append(
        created,
        element("span", `${pipeline.jobs.length} ${pipeline.jobs.length === 1 ? "job" : "jobs"}`),
      );
      ...
      graphScroll.setAttribute(
        "aria-label",
        `Job dependencies for pipeline on ${pipeline.branch} at ${shortCommitHash(pipeline.commit)}`,
      );
      ...
      stages.forEach((jobs, stageIndex) => {
        ...
        stageHeading.append(
          element("strong", stageIndex === 0 ? "Starts pipeline" : `Stage ${stageIndex + 1}`),
          element("span", `${jobs.length} ${jobs.length === 1 ? "job" : "jobs"}`),
        );
        ...
        for (const build of jobs) {
          ...
          metadata.append(
            element("span", buildDuration(build)),
            element("span", build.image ? `Image ${build.image}` : "No image"),
          );
          ...
          const logButton = actionButton(
            "View log",
            "file",
            "secondary icon-button build-log-toggle",
          );
  ...
  async function refreshBuilds(): Promise<void> {
    ...
    try {
      const previouslyActiveLog = activeLogBuildID !== null && data.builds.some(
        (build) =>
          build.id === activeLogBuildID &&
  ...
  const resizeHandler = (): void => scheduleDependencyLines();
  document.addEventListener("visibilitychange", visibilityHandler);
...
function repositoryBranchDivergence(branch: RepositoryBranch): string {
  if (!branch.compared) {
...
function repositoryBranchManager(
  route: RepositoryBrowserRoute,
  data: RepositoryBranches,
): {trigger: HTMLButtonElement; dialog: HTMLDialogElement} {
  ...
  if (data.branches.length === 0) {
    ...
    let activeConfirmation: HTMLTableRowElement | null = null;
    let activeDelete: HTMLButtonElement | null = null;
    ...
    for (const branch of data.branches) {
      ...
      if (branch.name === data.defaultBranch) {
        ...
        remove.addEventListener("click", () => {
          ...
          const warning = element(
            "p",
            `Delete ${branch.name}? The branch pointer will be removed and this cannot be undone from the UI.`,
          );
          const controls = element("div");
          ...
          form.addEventListener("submit", async (event) => {
            ...
            form.setAttribute("aria-busy", "true");
            try {
              const deleted = await request<RepositoryBranchDeletion>(
                repositoryBranchAPIURL(route.repository, branch.name),
                {
                  method: "DELETE",
                  headers: {"Content-Type": "application/json"},
              ...
              if (route.ref === branch.name) {
                ...
                const notice = element(
                  "p",
                  "The branch currently being viewed was deleted. Closing this window returns to the repository default.",
                );
                notice.className = "branch-manager-notice";
                description.after(notice);
              }
              showStatus(
                `${deleted.name} deleted at ${shortCommitHash(deleted.commit)}.`,
              );
            } catch (reason) {
              showStatus(
                reason instanceof Error ? reason.message : "Could not delete the branch.",
                true,
              );
...
function issueStateBadge(state: RepositoryIssue["state"]): HTMLElement {
  const badge = element("span");
  badge.className = `merge-request-state merge-request-state-${state}`;
  const label = state[0].toUpperCase() + state.slice(1);
  badge.append(
    icon(state === "closed" ? "close" : "clock"),
    document.createTextNode(label),
  );
...
async function showUpdatedIssue(
  route: RepositoryBrowserRoute,
  updated: RepositoryIssue,
  message: string,
): Promise<void> {
  const nextRoute: RepositoryBrowserRoute = {
    ...route,
...
function issueLabels(labels: string[], empty: string): HTMLElement {
...
function issueListItem(
  ...
  const updated = element(
    "span",
    `${issue.author} updated ${relativeTime(issue.updatedAt)}`,
  );
...
function repositoryIssueCreateForm(
  route: RepositoryBrowserRoute,
  ...
  form.addEventListener("submit", async (event) => {
    ...
    form.setAttribute("aria-busy", "true");
    try {
      const created = await request<RepositoryIssue>(
        repositoryIssuesAPIURL(route.repository),
        {
          method: "POST",
          headers: {"Content-Type": "application/json"},
          body: JSON.stringify({
...
async function repositoryIssueList(
  route: RepositoryBrowserRoute,
  canWrite: boolean,
): Promise<HTMLElement> {
  const data = await request<RepositoryIssues>(
    `${repositoryIssuesAPIURL(route.repository)}?state=all`,
  );
  ...
  section.className = "content-section merge-request-list-view";
  const actions: HTMLElement[] = [];
  let creation: HTMLFormElement | null = null;
  if (canWrite) {
  ...
  for (const state of ["open", "closed", "all"] as RepositoryIssueState[]) {
...
async function issueDescription(issue: RepositoryIssue): Promise<HTMLElement> {
  const card = element("section");
  card.className = "merge-request-description";
  const header = element("header");
...
async function issueComment(comment: RepositoryIssueComment): Promise<HTMLElement> {
  const card = element("article");
  card.className = "merge-request-comment";
  const header = element("header");
...
function issueCommentForm(
  ...
  form.addEventListener("submit", async (event) => {
    ...
    form.setAttribute("aria-busy", "true");
    try {
      await request<RepositoryIssue>(
        repositoryIssueCommentsAPIURL(route.repository, issue.id),
        {
          method: "POST",
          headers: {"Content-Type": "application/json"},
          body: JSON.stringify({body: body.value}),
      ...
      showStatus(
        reason instanceof Error ? reason.message : "Could not add the comment.",
        true,
      );
      submit.disabled = false;
      form.removeAttribute("aria-busy");
...
function issueDetailPanel(
  ...
  if (issue.state === "closed" && issue.closedAt) {
    const closed = element(
      "p",
      `Closed by ${issue.closedBy ?? "unknown"} ${relativeTime(issue.closedAt)}.`,
    );
  ...
  if (issue.canUpdate) {
    const nextState: RepositoryIssue["state"] = issue.state === "closed" ? "open" : "closed";
    const update = actionButton(
      nextState === "open" ? "Reopen issue" : "Close issue",
      nextState === "open" ? "refresh" : "close",
      nextState === "open" ? "primary" : "danger-secondary",
    );
    update.addEventListener("click", async () => {
      ...
      details.setAttribute("aria-busy", "true");
      try {
        const updatedIssue = await request<RepositoryIssue>(
          repositoryIssuesAPIURL(route.repository, issue.id),
          {
            method: "PATCH",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({state: nextState}),
        ...
        showStatus(
          reason instanceof Error ? reason.message : "Could not update the issue.",
          true,
        );
        update.disabled = false;
        details.removeAttribute("aria-busy");
...
async function repositoryIssueDetail(
  ...
  const opened = element(
    "p",
    `${issue.author} opened this issue ${relativeTime(issue.createdAt)}`,
  );
  opened.title = new Date(issue.createdAt).toLocaleString();
...
function repositoryFileCreator(
  ...
  form.append(
    ...
    fieldLabel("File contents", contentEditor.container),
...
function sourcePreview(content: string, path: string): HTMLElement {
...
async function renderEmptyRepositoryBrowser(
  route: RepositoryBrowserRoute,
  branches: RepositoryBranches,
  group: GroupDetail,
  openCounts: RepositoryOpenCounts | null,
  ...
  if (route.view !== "issues") {
    ...
    content.append(emptyState(
      "This repository has no browsable default. Push a commit to create its first branch.",
    ));
  }
  app.append(
...
function renderLogin(message = ""): void {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    error.hidden = true;
    try {
      const session = await request<BrowserSession>("/api/session", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({
          username: username.value.trim(),
