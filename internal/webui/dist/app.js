...
function setRepositoryLocation(repository, ref = "main") {
...
function repositoryContentBreadcrumbs(route) {
...
async function repositoryBlobSection(route, content) {
    ...
    const heading = sectionHeading(content.path, undefined, fileActions);
...
async function renderGroup(path, message) {
    ...
    app.append(pageHeader("Group", "", data.description, pageActions), subgroups, repositories, danger, createSubgroup.dialog, createRepository.dialog, ...(settingsControl ? [settingsControl.dialog] : []));
    if (message) {
        showStatus(message);
    }
}
