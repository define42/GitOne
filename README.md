...
The `{repository}`, `{ref}`, and in-repository `{path}` parameters are URL-encoded as individual path segments. Blob responses use UTF-8 text when possible and base64 for binary content. The web UI highlights recognized UTF-8 source files client-side with Prism. Tree `canEdit` enables file creation, while blob `canEdit` enables text editing and `canManage` enables rename/delete; these capabilities require a named branch and developer access. Browsable files are limited to 10 MiB.
...
| `GET` | `/api/repositories/{repository}/issues` | List persisted issues, filtered with `state=open`, `closed`, or `all`. Each issue includes its Git-safe `suggestedBranch`. |
| `POST` | `/api/repositories/{repository}/issues` | Create an issue. JSON fields: `title`, optional `description`, `labels`, and `assignees`. |
| `GET` | `/api/repositories/{repository}/issues/{id}` | Get an issue with its labels, assignees, comments, Git-safe `suggestedBranch`, and persisted branch metadata. |
...
| `POST` | `/api/repositories/{repository}/issues/{id}/branch` | Create and persist the issue branch. JSON fields: `name` and source branch `from`. |
